...
export const Default: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await expect(args.onClickItem).toHaveBeenCalledWith("save-as");
...
export const PrimaryAndAlternativeActions: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await expect(args.onClickItem).toHaveBeenCalledWith("run-selected");
...
export const MenuWithManyActions: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    await userEvent.keyboard("[ArrowUp]");
    await userEvent.keyboard("[Enter]");
    await expect(args.onClickItem).toHaveBeenCalledWith("option-10");
    await expect(canvas.queryByRole("menu")).not.toBeInTheDocument();
  },
};
