...
const meta: Meta<typeof KdsDropdown> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "Single selection dropdown with a popover list and search field. " +
...
export const WithValue: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // Option A has an icon accessory shown on the trigger's leading container
    const leading = canvasElement.querySelector(".leading");
    await expect(leading).not.toBeNull();
    await expect(leading!.querySelector(".kds-icon")).not.toBeNull();
  },
};
