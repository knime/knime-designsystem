...
9. **Never use `:deep()` to style child components.** Instead, remap legacy CSS custom properties on a parent wrapper element — CSS variables cascade naturally into all descendants regardless of scoped styles. For example, to tokenize a legacy `@knime/components` child that uses `--knime-masala`, set `--knime-masala: var(--kds-color-text-and-icon-neutral)` on the parent. This avoids coupling to internal class names and keeps styles maintainable.
10. **Edit components directly when possible.** If a component lives in the project (not `node_modules`), edit its styles in place. If a legacy component lives in `node_modules` and cannot be edited, prefer the CSS variable cascade (rule 9). Only as a last resort, copy the component into the project, convert it to use KDS tokens directly, and import the local copy.
